...
local Epsilon11Info = {
    ...
    team = "MTF",
    reasons = {"SCP939", "SCP049", "SCP173", "SCP096", "SCP035", "SCP079", "SCP035"}
}
        ...
        if file then
            ...
            if importedTable ~= nil then
                if importedTable.groups ~= nil then
                    groups = mergeTables(groups, importedTable.groups)
                end
            end
        end
        ...
        for player, role in pairs(roles) do
            ...
            if group.haveEquipment ~= nil then    
                if getIndex(group.haveEquipment, role) ~= -1 then
                    ...
                    if getIndex(scps, "SCP-106") ~= -1 then
                        player:GiveItem("ContainmentSpecialistCard")
                    end
                end
            end
        end
